...
router.get('/', authMiddleware_1.authenticateJWT, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        requisicoes = requisicoes.map(r => ({
            ...
            comentarioAdmin: r.comentarioAdmin || null,
            comentarioGestorDADM: r.comentarioGestorDADM || null,
            comentarioAprovacao: r.comentarioAprovacao || null,
            lastActionBy: r.lastActionBy || null,
            lastActionRole: r.lastActionRole || null,
            // ...existing code...
...
router.put('/:id/approve', authMiddleware_1.authenticateJWT, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (user.role === 'GESTOR_DADM') {
            ...
            requisicao.status = Requisicao_1.StatusRequisicao.AGUARDANDO_APROV_FINAL;
            yield requisicaoRepository.save(requisicao);
            return res.status(200).json(requisicao);
        }
        else if (user.role === 'ADMIN') {
            if (requisicao.status !== Requisicao_1.StatusRequisicao.AGUARDANDO_APROV_FINAL) {
...
router.put('/:id/status', authMiddleware_1.authenticateJWT, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!requisicao)
            return res.status(404).json({ message: 'Requisition not found.' });
        // Approval by Manager
        if (user.role === 'GESTOR_DADM' && requisicao.status === Requisicao_1.StatusRequisicao.PENDENTE && (newStatus === 'APROVADA_MANAGER' || newStatus === 'AGUARDANDO_APROV_FINAL')) {
        ...
        else if (user.role === 'ADMIN' && (requisicao.status === Requisicao_1.StatusRequisicao.APROVADA_GERENCIA || requisicao.status === Requisicao_1.StatusRequisicao.AGUARDANDO_APROV_FINAL || requisicao.status === Requisicao_1.StatusRequisicao.PENDENTE) && newStatus === 'APROVADA_FINAL') {
        ...
        // Rejection by Manager or Admin
            ...
            (user.role === 'ADMIN' && (requisicao.status === Requisicao_1.StatusRequisicao.APROVADA_GERENCIA || requisicao.status === Requisicao_1.StatusRequisicao.AGUARDANDO_APROV_FINAL || requisicao.status === Requisicao_1.StatusRequisicao.PENDENTE) && newStatus === 'REJEITADA')) {
            requisicao.status = Requisicao_1.StatusRequisicao.REJEITADA;
