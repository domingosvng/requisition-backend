...
router.get('/', authenticateJWT, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (user.role === 'SOLICITANTE') {
      query = query.where('solicitante.id = :userId', { userId: user.id });
...
// Approve a requisition (Manager only - first level approval)
router.put('/:id/approve', authenticateJWT, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    // Only managers can approve (first level)
    if (user.role === 'GESTOR_DADM') {
      if (requisicao.status !== StatusRequisicao.PENDENTE) {
        return res.status(403).json({ message: 'Requisition is not in the correct status for approval (Expected: PENDENTE).' });
      }
      requisicao.status = StatusRequisicao.AGUARDANDO_APROV_FINAL;
      if (comentario) requisicao.comentarioAdmin = comentario;
    ...
    return res.status(403).json({ message: 'Only managers can approve requisitions.' });
  } catch (error) {
...
// Accept a requisition (Admin only - final acceptance)
router.put('/:id/accept', authenticateJWT, async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
    const { comentario } = req.body;
    const user = req.user;
    ...
    // Only admins can accept (final approval)
    if (user.role === 'ADMIN') {
      // Allow admin to accept PENDENTE, AGUARDANDO_APROV_FINAL, or APROVADA_GERENCIA
      if (![StatusRequisicao.PENDENTE, StatusRequisicao.AGUARDANDO_APROV_FINAL, StatusRequisicao.APROVADA_GERENCIA].includes(requisicao.status)) {
        return res.status(403).json({ message: 'Requisition cannot be accepted in its current status.' });
      }
      
      // Add notification if bypassing gestor approval
      let notificationMessage = '';
      if (requisicao.status === StatusRequisicao.PENDENTE) {
        notificationMessage = 'Note: Manager approval was bypassed by Admin.';
      }
    ...
    return res.status(403).json({ message: 'Only admins can accept requisitions.' });
  } catch (error) {
    console.error('Erro ao aceitar requisição:', error);
    res.status(500).json({ message: 'Erro interno do servidor ao aceitar requisição.' });
  }
});
...
router.put('/:id/reject', authenticateJWT, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (user.role === 'GESTOR_DADM') {
      ...
      // Allow admin to reject PENDENTE, AGUARDANDO_APROV_FINAL, or APROVADA_GERENCIA
      if (![StatusRequisicao.PENDENTE, StatusRequisicao.AGUARDANDO_APROV_FINAL, StatusRequisicao.APROVADA_GERENCIA].includes(requisicao.status)) {
        return res.status(403).json({ message: 'Requisition cannot be rejected in its current status.' });
      }
      
      // Add notification if bypassing gestor review
      let notificationMessage = '';
      if (requisicao.status === StatusRequisicao.PENDENTE) {
        notificationMessage = 'Note: Manager review was bypassed by Admin rejection.';
      }
