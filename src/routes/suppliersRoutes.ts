...
router.post('/',
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!errors.isEmpty()) {
      // Map to concise field/message shape with more detailed error information
    ...
    try {
      ...
      if (nif) {
        ...
        if (existing) return res.status(409).json({ 
          message: 'Já existe um fornecedor com esse NIF.',
          errors: [{ field: 'nif', message: 'NIF já está em uso', value: nif }],
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      if (updateData && updateData.nif && updateData.nif !== fornecedor.nif) {
        ...
        if (existing) return res.status(409).json({ 
          message: 'Já existe um fornecedor com esse NIF.',
          errors: [{ field: 'nif', message: 'NIF já está em uso', value: updateData.nif }],
          errorType: 'DUPLICATE_VALUE'
        });
      }
